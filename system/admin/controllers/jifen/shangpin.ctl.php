...
class Ctl_Jifen_Shangpin extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'admin:jifen/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:jifen/so.html';
    }
    ...
    public function detail($product_id = null)
    {
        ...
        }else if(!$detail = K::M('jifenshangcheng/shangpin')->detail($product_id)){
            $this->err->add('您要查看的内容不存在或已经删除', 212);
        }else if(!$this->check_city($detail['city_id'])){
                $this->err->add('不可越权操作', 403);
        }else{
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'admin:jifen/detail.html';
        }
    }
    ...
    public function create($cat_id=null)
    {
        if($data = $this->checksubmit('data')){ 
            ...
            if($product_id = K::M('jifenshangcheng/shangpin')->create($data)){
                $this->err->add('添加内容成功');
                $this->err->set_data('forward', '?jifen/shangpin-index.html');
            } 
        }else{
            if($cat_id){
                $pager['cat_id'] = $cat_id;
            ...
            $this->tmpl = 'admin:jifen/create.html';
        }
    }
    ...
    public function edit($product_id=null,$cat_id=null)
    {
        if(!($product_id = (int)$product_id) && !($product_id = $this->GP('product_id'))){
            ...
            if(K::M('jifenshangcheng/shangpin')->update($product_id, $data)){
                $this->err->add('修改内容成功');
            }  
        }else{
            ...
            $this->cookie->set('LAST_product_pids', K::M('shop/cate')->parent_ids($cat_id, ',', true));
        	$this->pagedata['detail'] = $detail;
        	$this->tmpl = 'admin:jifen/edit.html';
        }
    }
    ...
    {
        if($product_id = (int)$product_id){
            if(K::M('jifenshangcheng/shangpin')->batch($product_id, array('audit'=>1))){
                $this->err->add('审核内容成功');
            }
        }else if($ids = $this->GP('product_id')){
            if(K::M('jifenshangcheng/shangpin')->batch($ids, array('audit'=>1))){
    ...
    public function delete($product_id=null)
    {
        if($product_id = (int)$product_id){
            if(!$detail = K::M('jifenshangcheng/shangpin')->detail($product_id)){
                ...
                if(K::M('jifenshangcheng/shangpin')->delete($product_id)){
                    $this->err->add('删除内容成功');
                }
            }
        }else if($ids = $this->GP('product_id')){
            if($items = K::M('jifenshangcheng/shangpin')->items_by_ids($ids)){
                $aids = array();
                foreach($items as $v){
                ...
                if(K::M('jifenshangcheng/shangpin')->delete($aids)){
    ...
    {
        $this->tmpl = 'admin:jifen/select_cate.html';
    } 
