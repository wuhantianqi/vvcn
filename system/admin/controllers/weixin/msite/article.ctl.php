...
class Ctl_Weixin_Msite_Article extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'admin:weixin/msite/article/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:weixin/msite/article/so.html';
    }
    ...
    public function create()
    {
        if($data = $this->checksubmit('data')){
            ...
            if($article_id = K::M('msite/article')->create($data)){
                $this->err->add('添加内容成功');
                $this->err->set_data('forward', '?weixin/msite/article-index.html');
            } 
        }else{
           $this->tmpl = 'admin:weixin/msite/article/create.html';
        }
    }
    ...
    public function edit($article_id=null)
    {
        if(!($article_id = (int)$article_id) && !($article_id = $this->GP('article_id'))){
        	...
        	$this->tmpl = 'admin:weixin/msite/article/edit.html';
        }
    }
