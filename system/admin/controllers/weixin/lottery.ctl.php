...
class Ctl_Weixin_Lottery extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('weixin/lottery')->items($filter, null, $page, $limit, $count)){
        	$pager['count'] = $count;
        	$pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array('{page}')), array('SO'=>$SO));
        }
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:weixin/lottery/items.html';
    }
    ...
    public function create()
    {
        if($data = $this->checksubmit('data')){
			...
			if(!$items = K::M('weixin/keyword')->items(array('keyword'=>$data['keyword'],'wx_id'=>$data['wx_id']))){
				if($id = K::M('weixin/lottery')->create($data)){
					$keyword = array();
					...
					$keyword['plugin'] = 'lottery:'.$id;
					K::M('weixin/keyword')->create($keyword);
					$this->err->add('添加内容成功');
					$this->err->set_data('forward', '?weixin/lottery-index.html');
				} 
			}else{
           ...
           $this->tmpl = 'admin:weixin/lottery/create.html';
        }
    }
    ...
    public function edit($id=null)
    {
        if(!($id = (int)$id) && !($id = $this->GP('id'))){
        ...
        }else if(!$detail = K::M('weixin/lottery')->detail($id)){
            ...
            if(K::M('weixin/lottery')->update($id, $data)){
                $this->err->add('修改内容成功');
            }  
        }else{
        	$this->pagedata['detail'] = $detail;
        	$this->tmpl = 'admin:weixin/lottery/edit.html';
        }
    }
    ...
    {
        if($id = (int)$id){
            if(!$detail = K::M('weixin/lottery')->detail($id)){
