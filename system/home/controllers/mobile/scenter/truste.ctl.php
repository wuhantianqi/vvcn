		...
		if(!($truste_id = (int)$truste_id) && !($truste_id = (int)$this->GP('truste_id'))){
            ...
            if(!$content = $data['content']){
                ...
                if($look_id = K::M('truste/look')->create($data)){
                    ...
                    $this->err->add('参加竞标成功！');
					$this->err->set_data('forward', $this->mklink('mobile/scenter/truste:trusteDetail', array('truste_id'=>$truste_id)));
                }
            }
        }else{
			$pager['backurl'] = $this->mklink('mobile/scenter/truste:trusteDetail', array('truste_id'=>$truste_id));
			$this->pagedata['pager'] = $pager;
