		...
		if(!($tenders_id = (int)$tenders_id) && !($tenders_id = (int)$this->GP('tenders_id'))){
            ...
            if(!$content = $data['content']){
                ...
                if($look_id = K::M('tenders/look')->create($data)){
                    ...
                    $this->err->add('参加竞标成功！');
					$this->err->set_data('forward', $this->mklink('mobile/dcenter/tenders:tenderDetail', array('tenders_id'=>$tenders_id)));
                }
            }
        }else{
			$pager['backurl'] = $this->mklink('mobile/dcenter/tenders-tendered', array('tenders_id'=>$tenders_id));
			$this->pagedata['pager'] = $pager;
