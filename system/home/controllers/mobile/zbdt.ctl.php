...
class Ctl_Mobile_Zbdt extends Ctl_Mobile
{
    ...
    public function index()
    {
        ...
        foreach($cate_list as $k=>$v){
            if($v['parent_id']==0){
                $v['link'] = $this->mklink('mobile/zbdt:index',array($k,$status,$order,1)); 
                $parents[$k] = $v;
            }
        }
        ...
        $cate_link = $this->mklink('mobile/zbdt:index',array($top_cate['cat_id'],$status,$order,1));
        $a_link = $this->mklink('mobile/zbdt:index',array($cat_id,0,$order,1));
        $b_link = $this->mklink('mobile/zbdt:index',array($cat_id,1,$order,1));
        $c_link = $this->mklink('mobile/zbdt:index',array($cat_id,2,$order,1));
        $order_alink = $this->mklink('mobile/zbdt:index',array($cat_id,$status,0,1));        
        $order_blink = $this->mklink('mobile/zbdt:index',array($cat_id,$status,1,1));        
        $order_clink = $this->mklink('mobile/zbdt:index',array($cat_id,$status,2,1));
        if($order == 0){
            $orderby = null;
        }else if($status == 1){//最新
        ...
        $price_link = $this->mklink('mobile/zbdt:index',array($cat_id,$status,$order,1)); 
        if($cat_id){
            $childeren = K::M('truste/cate')->children_ids($cat_id, $glue=',');
            $filter['cate_id'] = explode(',',$childeren);
        }
        $all_parent_cat_link = $this->mklink('mobile/zbdt:index',array(0,$status,$order,1));
        $limit = 10;
        $count = 0 ;
        if($items = K::M('truste/truste')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/zbdt:index', array($cat_id,$status,$order, '{page}')));
        ...
        $data = array('cat_id' =>$cat_id , 'status' => $status ,'order'=>$order,'pcat'=>$p_cat,'cateids' => $cateids,'top_cate'=>$top_cate);
